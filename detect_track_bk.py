...
class Yolo7Detect:
    ...
    def detect(self, save_img=False):
        ...
        encoder = gdet.create_box_encoder(deepsort_model, batch_size=1)
        ...
        for path, imgL, imgR, im0sL, im0sR, vid_cap in dataset:
            ...
            for i, det in enumerate(predL):  # detections per image
                ...
                if len(det):
                    ...
                    track_fish_list_L = track_fish_L.track_fish(im0L, bboxesL, scoresL)
                    ...
                    # predL_lst = merge_lists(predL_lst, track_fish_list_L)
                    '''
                ...
                for i, detR in enumerate(predR):  # detections per image
                    ...
                    if len(detR):
                        ...
                        # DeepSORT -> Saving Track predictions into a text file.
                        save_format = '{frame},{id},{x1},{y1},{w},{h},{x},{y},{z}\n'
                        print("bboxesR ", bboxesR)
                        print("scoresR ", scoresR)
                        track_fish_list_R = track_fish_R.track_fish(im0R, bboxesR, scoresR)
                        for tid in track_fish_list_R:
                            with open(txt_path + 'R_T.txt', 'a') as f:
                                line = save_format.format(frame=frame_idx, id=tid[0], x1=tid[1],
                                                          y1=tid[2],
                                                          w=tid[3], h=tid[4], x=-1, y=-1,
                                                          z=-1)
                        ...
                        predR_lst = merge_lists(predR_lst, track_fish_list_R)
                        '''
