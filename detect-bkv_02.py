...
from stereo_camera_manager.utils.triangulation import Triangulation as tri
...
class Yolo7Detect:
    ...
    def detect(self, save_img=False):
        ...
        for path, imgL, imgR, im0sL, im0sR, vid_cap in dataset:
            ...
            # img = imgL
            calib = Calibration(stereomap)

            # Stereo vision setup parameters
            frame_rate = 25  # 120    #Camera frame rate (maximum at 120 fps)
            B = 4.2  # 9               #Distance between the cameras [cm]
            f = 4  # 8              #Camera lense's focal length [mm]
            alpha = 84.8  # Camera field of view in the horisontal plane [degrees]

            im0sR, im0sL = calib.undistortRectify(im0sR, im0sL)   ## CALIBRATION
