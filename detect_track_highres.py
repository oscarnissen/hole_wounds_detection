            ...
            for i, det in enumerate(predL):  # detections per image
                ...
                if save_img:
                    if dataset.mode == 'image':
                        if stereomod:
                            print("----- image stereo mode ------------------")
                            # cv2.imwrite("parrot_saved.jpg", img, [cv2.IMWRITE_TIFF_RESUNIT, 300])
                            #cv2.imwrite(save_path, np.concatenate((
                            #    im0L, im0R
                            #), axis=1))
                            cv2.imwrite(save_path, np.concatenate((
                                im0L, im0R
                            ), axis=1), [cv2.IMWRITE_TIFF_RESUNIT, 300])
                        else:
                            cv2.imwrite(save_path, im0L, [cv2.IMWRITE_TIFF_RESUNIT, 300])
                        ...
                        # print("{:05d}".format(frame_idx))
                        cv2.imwrite(filename + "_img_{:05d}.png".format(frame_idx), np.concatenate((
                            im0L, im0R
                        ), axis=1), [cv2.IMWRITE_TIFF_RESUNIT, 300])
                        print(f" The image with the result is saved in: {filename}_img_{frame_idx}.png")
                        ####################################################
